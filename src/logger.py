...
class StructuredLogger:
    ...
    def __init__(self, name: str = "Forge"):
        ...
        json_log_file = os.path.join(LOGS_DIR, f"agent_metrics_{time.strftime('%Y%m%d')}.jsonl")
    ...
    def _log(self, level: str, msg: str, context: dict = None):
        ...
        try:
            json_record = logging.LogRecord(
                name=self.logger.name, level=logging.getLevelName(level), pathname="", lineno=0,
                msg=json.dumps(log_entry), args=(), exc_info=None
            )
