    ...
    def listen(self, duration=None, meeting_mode=False):
        ...
        try:
            with sd.InputStream(
                ...
                while self.is_recording:
                    ...
                    event_bus.publish(
                        "audio_telemetry", float(max_amp * 2.0)
                    )  # amplify for visual

                    if max_amp > self.noise_threshold:
                        speech_started = True
                        silence_frames = 0
                    else:
                        if speech_started:
                    ...
                    if not meeting_mode and len(audio_chunks) > int(
                        30 / chunk_duration
                    ):
                        break
            ...
            import time
            time.sleep(2) # Prevent CPU spinning if microphone is unavailable
        finally:
            self.is_recording = False
            event_bus.publish("audio_telemetry", 0.0)
        ...
        if self.native_sr != self.target_sr:
            ...
            target_times = np.linspace(
                0,
                len(full_audio) - 1,
                int(len(full_audio) * self.target_sr / self.native_sr),
            )
            full_audio = np.interp(target_times, original_times, full_audio).astype(
                np.float32
            )
...
def main():
    recognizer = SpeechRecognizer(
        model_size="large-v3-turbo", device="cpu", compute_type="int8"
    )
    while True:
        print("\nReady.")
