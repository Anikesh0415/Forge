...
class MemoryManager:
    ...
    def get_episodic_memory(self) -> list:
        """Retrieves user preferences and verified facts from episodic memory."""
        return self.long_term_memory.get("episodic_facts", [])

    def get_semantic_memory(self, query: str) -> str:
        """Retrieves relevant skills from Semantic RAG DB."""
        try:
            from src.utils.skill_retriever import retriever
            results = retriever.search_skills(query, top_k=2)
            if results:
                return "\n".join([doc for doc in results.get("documents", [[]])[0] if doc])
            return ""
        except Exception:
            return ""
